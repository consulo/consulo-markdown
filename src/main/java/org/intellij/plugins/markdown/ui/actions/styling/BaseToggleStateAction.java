...
public abstract class BaseToggleStateAction extends ToggleAction implements DumbAware
{
	...
	{
		return MarkdownActionUtil.getCommonParentOfType(element1, element2, getTargetNodeType()) == null ? SelectionState.NO : SelectionState.YES;
	}
			...
			public void run()
			{
				...
				for(Caret caret : ContainerUtil.reverse(editor.getCaretModel().getAllCarets()))
				{
					if(!state)
					{
						...
						final PsiElement closestEmph = MarkdownActionUtil.getCommonParentOfType(elements.getFirst(), elements.getSecond(), BaseToggleStateAction.this.getTargetNodeType());
						if(closestEmph == null)
						{
